...
public class WorldExporter extends AbsExporter
{
	...
	{
		// 打开所有装扮文件
		GamePacker.beginLogSet("读取装扮");
		for (AttireFile attireFile : attires.values())
		{
			GamePacker.progress("读取装扮", attireFile.getInnerPath());
	...
	private void syncScenes() throws Exception
	{
		...
		// 打开场景
		GamePacker.beginLogSet("读取场景");
		for (Scene scene : scenes.values())
		{
			GamePacker.progress("读取场景", scene.getInnerPath());
			scene.open(this);
		...
		for (Scene scene : scenes.values())
		{
					...
					for (AttireAnim anim : action.getAnims())
					{
						...
						for (int i = 0; i < regionCount; i++)
						{
							...
							if (delay > 0)
							{
								...
								{
									System.err.println("场景动画图像区域未换到！ "+scene.getInnerPath()+"     attireName="+attire.getKey()+"  action="+action.getID()+"  anim="+anim.getGroupID()+","+anim.getLayerID());
									//throw new Error("场景动画图像区域未换到！ "+scene.getInnerPath());
								}
							}
						}
						sb.append("\t\t\t\t</anim>\n");
			...
			for (SceneBackLayer layer : scene.getBackLayers())
			{
				if (layer.getImage().getFile() != null && layer.getImage().getFile().exists())
				{
					...
					{
						//throw new Error("背景图层图像区域未换到！ "+scene.getInnerPath());
						System.err.println("背景图层图像区域未换到！ "+scene.getInnerPath()+"     layerName="+layer.getName()+"  img="+layer.getImage().getInnerpath());
					}
				}
			}
			sb.append("\t</layers>\n");
			...
			for (SceneForeLayer layer : scene.getForeLayers())
			{
				if (layer.getImage().getFile() != null && layer.getImage().getFile().exists())
				{
					...
					{
						//throw new Error("自动图层图像区域未换到！ "+scene.getInnerPath());
						System.err.println("自动图层图像区域未换到！ "+scene.getInnerPath()+"     layerName="+layer.getName()+"  img="+layer.getImage().getInnerpath());
					}
				}
			}
			sb.append("\t</foreLayers>\n");
