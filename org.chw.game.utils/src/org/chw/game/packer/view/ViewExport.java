package org.chw.game.packer.view;
...
public class ViewExport extends AbsExporter
{
	...
	public void addOutputBag(ViewFileBag bag) throws Exception
	{
		...
		{
			String url=exportFile(bag.getKey(), bag.build(), "swf");
			if(url.indexOf("343.swf")!=-1)
			{
	...
	private void writeDB() throws Exception
	{
		...
		for (ViewFile file : list)
		{
			if (file.isCfg())
			{
				...
				for (ViewFileGroup group : file.getDependViewFileGroups())
				{
					for (ViewFileBag bag : group.getBags())
					{
						if (!swfKeys.contains(bag.getKey()))
						{
							...
							if (file.getPreloadMethod() != -1)
							{
								int size = 0;
								int method = file.getPreloadMethod();
				...
				// 记录信息
				descXML.append("\t\t<view preload=\"" + file.getPreloadMethod() + "\" name=\"" + file.getBagName() + "\" url=\"" + url + "\" id=\"FILE_" + file.getClassID() + "\" size=\"" + bagSize + "\" files=\"" + urls.toString() + "\"/>\n");
			}
		}
		descXML.append("\t</views>\n");
		...
		for (Integer method : methodIDs)
		{
			...
			for (String path : paths)
			{
				...
				//File viewFrom = new File(getDestDir().getParentFile().getPath() + path);
				//File viewDest = new File(getDestDir().getPath() + "/.ver/preloads/" + method + "/" + viewFrom.getName());
