...
import org.chw.game.AppSetting;
import org.chw.game.packer.cfg.ConfigExporter;
import org.chw.game.packer.files.FilesExporter;
import org.chw.game.packer.game.GameExporter;
import org.chw.game.packer.icon.IconExporter;
import org.chw.game.packer.view.ViewExport;
...
public class GamePacker extends Composite
{
	...
	public GamePacker(Composite parent, int style)
	{
		...
		GridLayout gl_params = new GridLayout(3, false);
		gl_params.marginWidth = 20;
		...
		mobileSelection.setText("\u4F7F\u7528\u624B\u673A\u914D\u7F6E\u683C\u5F0F");

		clearupSelection = new Button(params, SWT.CHECK);
		clearupSelection.setText("\u6E05\u7406\u65E7\u6587\u4EF6");
	...
	private void execExport()
	{
		...
		execThread = new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				if (worldSelected)
				{
					WorldExporter world = new WorldExporter(new File(worldPath), new File(cdnPath + File.separatorChar + "world"),zip,isMobile);
					world.publish();
				}
